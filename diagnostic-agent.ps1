...
Function Download-And-Install-ProcExplorer {
    ...
    if (Test-InternetConnection) {
        ...
        Expand-Archive -Path "$downloadFolder\ProcessExplorer.zip" -DestinationPath $downloadFolder -Force # Extract Process Monitor ZIP
    }
}
...
try {
    ...
    Move-Item -Path ".\$procexpCaptureFile" -Destination $capturedLogsDirectory # Rename and move the capture file
}
catch {
    Write-Host "An error occurred: $_"
}
